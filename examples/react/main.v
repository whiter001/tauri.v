// main.v — vtauri + React 示例应用
//
// 前端：examples/react/frontend（Vite + React），构建为单个 index.html 后在此嵌入。
//
// 两种加载模式：
...
//      bash ../../scripts/build_example_frontends.sh react
//   2. Windows 编译（MSVC，V 自动编译 native/webview_bridge.cpp 为 .obj 并链接）：
//      powershell -ExecutionPolicy Bypass -File ../../scripts/build_examples_msvc.ps1 -Example react
//   3. 运行 react.exe（目标机需安装 WebView2 Runtime）
//
// 开发模式（前端热更新）：
...
fn main() {
	...
	app.register_command('info', fn (args string) !string {
		return '{"app":"vtauri-react","version":"0.1.0","backend":"V 0.5.2","commands":["greet","add","info"]}'
	})
